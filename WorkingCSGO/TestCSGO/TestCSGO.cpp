...
DWORD modName(LPSTR moduleName, DWORD pid)
{
...
HANDLE GetProcess(const char* name, PROCESSENTRY32 *procEntry)
{
	pid = 0;
	...
	do
	{
		if (!strcmp(process32.szExeFile, name))
		{
...
DWORD WINAPI Main(LPVOID Params) {
	...
	try{ process = GetProcess("csgo.exe", &gameProcess); }
	...
	while (client == 0x0)
	{
		...
		client = modName("client.dll", gameProcess.th32ProcessID);

	}
	while (engine == 0x0)
	{
		engine = modName("engine.dll", gameProcess.th32ProcessID);
	...
	{
		MessageBox(NULL, "Couldnt register", "Problem", MB_OK);
		return 0;
	}
	...
	if (EspHWND == NULL)
	{
		MessageBox(NULL, "Window could be created", "Problem", MB_OK);
	}
